...
// Reemplaza esta dirección con la de tu contrato desplegado en Remix
...
export default function Home() {
  ...
  let configureBlockchain = async () => {
    try {
      ...
      if (provider) {
        ...
        // Comprobar que estamos en la blockchain correcta (BSC Testnet)
        // En metamask debemos dar permiso solo a esa a localhost:3000 para que no haya problemas
        const chainId = await provider.request({ method: 'eth_chainId' });
        console.log('Red conectada (chainId):', chainId, '→ decimal:', parseInt(chainId, 16));
        // BSC Testnet = 0x61 (97 decimal)
        if (chainId !== '0x61') {
          alert(`Red incorrecta. MetaMask está en chainId ${parseInt(chainId, 16)}.\nCambia a BSC Testnet (chainId 97).`);
          return;
        }
  ...
  let cargarTareas = async () => {
    ...
    try {
      ...
      const lista = descs.map((desc, i) => ({
        id: i, descripcion: desc, completada: completadas[i], creador: creadores[i]
      }));
      setTareas(lista);
    } catch (error) {
  ...
  let handleCompletarTarea = async (index) => {
    if (!myContract.current) return;
    try {
  ...
  return (
    <div style={{ padding: '2rem', fontFamily: 'sans-serif' }}>
      ...
      <ul style={{ listStyle: 'none', padding: 0 }}>
        {tareas.map((tarea) => (
          <li key={tarea.id} style={{ marginBottom: '0.75rem', padding: '0.75rem', border: '1px solid #ddd', borderRadius: '6px' }}>
            <strong>#{tarea.id}</strong> — {tarea.descripcion}{' '}
            {tarea.completada
              ? <span>✅ Completada</span>
              : tarea.creador.toLowerCase() === cuenta.toLowerCase()
                ? <button onClick={() => handleCompletarTarea(tarea.id)} style={{ marginLeft: '0.5rem' }}>Completar</button>
                : <span>⏳ Pendiente</span>
            }
            <br />
